...
import { personnelEmailTemplateSchema } from "@/lib/validations";
...
export async function PUT(request: NextRequest) {
  ...
  const parsed = personnelEmailTemplateSchema.safeParse(body);
  if (!parsed.success) {
  ...
  await logAudit({
    ...
    description: `Mengubah template email reminder personil (mode ${parsed.data.mode === "SIMPLE" ? "Sederhana" : "Lanjutan"}).`,
  });
