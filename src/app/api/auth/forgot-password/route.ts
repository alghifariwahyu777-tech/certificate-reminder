...
export async function POST(request: NextRequest) {
  ...
  if (transporter) {
    ...
    await transporter
      .sendMail({
        ...
        to: user.email,
        subject: "Reset Password - Certificate Reminder Admin",
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; color: #1e293b;">
            <div style="background: #0F172A; padding: 20px 24px; border-radius: 8px 8px 0 0; border-bottom: 3px solid #0EA89B;">
              <span style="color: #ffffff; font-size: 14px; font-weight: bold;">PT Sucofindo (Persero) — Certificate Reminder System</span>
            </div>
            <div style="border: 1px solid #e2e8f0; border-top: none; border-radius: 0 0 8px 8px; padding: 24px;">
              <p>Halo ${user.name},</p>
              <p>Kami menerima permintaan reset password untuk akun Admin Anda. Klik tombol di bawah untuk membuat password baru (tautan berlaku ${TOKEN_VALID_MINUTES} menit):</p>
      ...
      .catch((err) => console.error("Failed to send admin reset password email:", err));
  }
  ...
  await logAudit({
    ...
    action: "UPDATE",
    entityType: "Auth",
    description: `Permintaan reset password untuk akun Admin ${user.email}.`,
  });
