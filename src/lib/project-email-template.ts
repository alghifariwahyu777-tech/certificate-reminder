...
import type { ProjectReminderEmailData } from "@/lib/project-email";

export type { ProjectReminderEmailData } from "@/lib/project-email";

export type ProjectSimpleTemplateFields = {
  companyName: string;
  systemName: string;
  greeting: string;
  introText: string;
...
export const PROJECT_PLACEHOLDER_TOKENS: { token: string; label: string }[] = [
  { token: "{{pic}}", label: "Nama PIC" },
  { token: "{{projectName}}", label: "Nama Project" },
  { token: "{{projectNumber}}", label: "Nomor Project" },
  { token: "{{categoryName}}", label: "Kategori / Portofolio" },
  { token: "{{clientName}}", label: "Klien / Pemberi Kerja" },
  { token: "{{targetEndDate}}", label: "Target Selesai" },
...
export const DEFAULT_PROJECT_SUBJECT = "[Project Monitoring] Target Selesai Project Akan Tercapai";

export const DEFAULT_PROJECT_SIMPLE_FIELDS: ProjectSimpleTemplateFields = {
  companyName: "PT SUCOFINDO (Persero)",
  systemName: "Project Monitoring Reminder",
  greeting: "Halo {{pic}},",
  introText: "Ini pengingat bahwa target penyelesaian project berikut akan segera tercapai atau telah terlewati.",
  closingText:
    "Mohon segera koordinasikan progres penyelesaian, atau ajukan addendum bila diperlukan perpanjangan waktu.",
...
function tokenValues(data: ProjectReminderEmailData, appUrlOverride?: string): Record<string, string> {
  ...
  return {
    ...
    projectNumber: escapeHtml(data.projectNumber),
    categoryName: escapeHtml(data.categoryName),
    clientName: escapeHtml(data.clientName),
    targetEndDate: formatDate(data.targetEndDate),
...
export function substituteProjectPlaceholders(
  template: string,
...
export function buildProjectSimpleTemplateHtml(
  fields: ProjectSimpleTemplateFields,
  data: ProjectReminderEmailData,
  appUrlOverride?: string
): string {
  const values = tokenValues(data, appUrlOverride);
  const t = (text: string) =>
    escapeHtml(substituteProjectPlaceholders(text, data, appUrlOverride)).replace(/\n/g, "<br/>");
  const companyName = escapeHtml(fields.companyName || DEFAULT_PROJECT_SIMPLE_FIELDS.companyName);
  const systemName = escapeHtml(fields.systemName || DEFAULT_PROJECT_SIMPLE_FIELDS.systemName);
...
<html lang="id">
  <head>
    ...
    <title>Project Monitoring Reminder</title>
  </head>
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background-color:#ffffff;border:1px solid #E2E8F0;border-radius:8px;overflow:hidden;">
            ...
            <tr>
              <td style="padding:32px;">
                <p style="margin:0 0 16px 0;color:#0F172A;font-size:14px;">${t(fields.greeting || DEFAULT_PROJECT_SIMPLE_FIELDS.greeting)}</p>
                <p style="margin:0 0 20px 0;color:#334155;font-size:14px;line-height:1.6;">${t(fields.introText || DEFAULT_PROJECT_SIMPLE_FIELDS.introText)}</p>
                ...
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #E2E8F0;border-radius:6px;margin-bottom:20px;">
                  <tr>
                    <td style="padding:10px 16px;border-bottom:1px solid #E2E8F0;color:#64748B;font-size:13px;width:40%;">Nama Project</td>
                    <td style="padding:10px 16px;border-bottom:1px solid #E2E8F0;color:#0F172A;font-size:13px;font-weight:bold;">${values.projectName}</td>
                  </tr>
                  <tr>
                    <td style="padding:10px 16px;border-bottom:1px solid #E2E8F0;color:#64748B;font-size:13px;">Nomor Project</td>
                    <td style="padding:10px 16px;border-bottom:1px solid #E2E8F0;color:#0F172A;font-size:13px;font-weight:bold;">${values.projectNumber}</td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    <td style="padding:10px 16px;border-bottom:1px solid #E2E8F0;color:#64748B;font-size:13px;">Target Selesai</td>
                    <td style="padding:10px 16px;border-bottom:1px solid #E2E8F0;color:${values.statusColor};font-size:13px;font-weight:bold;">${values.targetEndDate}</td>
                  </tr>
                  <tr>
                ...
                <p style="margin:0 0 8px 0;color:#334155;font-size:14px;line-height:1.6;">${t(fields.closingText || DEFAULT_PROJECT_SIMPLE_FIELDS.closingText)}</p>
                <p style="margin:24px 0 0 0;color:#334155;font-size:14px;">Terima kasih.</p>
              </td>
            </tr>
            <tr>
              <td style="padding:20px 32px;background-color:#F8FAFC;border-top:1px solid #E2E8F0;">
                <p style="margin:0;color:#94A3B8;font-size:11px;line-height:1.6;">${t(fields.footerText || DEFAULT_PROJECT_SIMPLE_FIELDS.footerText)}</p>
              </td>
            </tr>
          </table>
        </td>
...
export const DEFAULT_PROJECT_BODY_HTML = `<!DOCTYPE html>
<html lang="id">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
      ...
      <tr>
        <td align="center">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background-color:#ffffff;border:1px solid #E2E8F0;border-radius:8px;overflow:hidden;">
                ...
                <table role="presentation" cellpadding="0" cellspacing="0">
                  <tr>
                    ...
                    <td style="padding-left:12px;">
                      ...
                      <br />
                      <span style="color:#0EA89B;font-size:13px;letter-spacing:0.1em;text-transform:uppercase;">Project Monitoring Reminder</span>
                    </td>
                  </tr>
                </table>
              </td>
            ...
            <tr>
              <td style="padding:32px;">
                <p style="margin:0 0 16px 0;color:#0F172A;font-size:14px;">Halo {{pic}},</p>
                <p style="margin:0 0 20px 0;color:#334155;font-size:14px;line-height:1.6;">Ini pengingat bahwa target penyelesaian project berikut akan segera tercapai atau telah terlewati.</p>

                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #E2E8F0;border-radius:6px;margin-bottom:20px;">
                  <tr>
                    <td style="padding:10px 16px;border-bottom:1px solid #E2E8F0;color:#64748B;font-size:13px;width:40%;">Nama Project</td>
                    <td style="padding:10px 16px;border-bottom:1px solid #E2E8F0;color:#0F172A;font-size:13px;font-weight:bold;">{{projectName}}</td>
                  </tr>
                  <tr>
                    <td style="padding:10px 16px;border-bottom:1px solid #E2E8F0;color:#64748B;font-size:13px;">Nomor Project</td>
                    <td style="padding:10px 16px;border-bottom:1px solid #E2E8F0;color:#0F172A;font-size:13px;font-weight:bold;">{{projectNumber}}</td>
                  </tr>
                  <tr>
                    <td style="padding:10px 16px;border-bottom:1px solid #E2E8F0;color:#64748B;font-size:13px;">Kategori</td>
                  ...
                  <tr>
                    <td style="padding:10px 16px;border-bottom:1px solid #E2E8F0;color:#64748B;font-size:13px;">Target Selesai</td>
                    <td style="padding:10px 16px;border-bottom:1px solid #E2E8F0;color:{{statusColor}};font-size:13px;font-weight:bold;">{{targetEndDate}}</td>
                  </tr>
                  <tr>
                    <td style="padding:10px 16px;color:#64748B;font-size:13px;">Sisa Hari</td>
                ...
                <p style="margin:0 0 8px 0;color:#334155;font-size:14px;line-height:1.6;">Mohon segera koordinasikan progres penyelesaian, atau ajukan addendum bila diperlukan perpanjangan waktu.</p>
                <p style="margin:24px 0 0 0;color:#334155;font-size:14px;">Terima kasih.</p>
              </td>
            </tr>
            <tr>
