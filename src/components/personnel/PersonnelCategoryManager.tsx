...
type PersonnelCategoryItem = { id: string; name: string; certificationCount: number };

export function PersonnelCategoryManager({
  initialCategories,
  canManage = true,
}: {
  initialCategories: PersonnelCategoryItem[];
  canManage?: boolean;
}) {
  ...
  const [editTarget, setEditTarget] = useState<PersonnelCategoryItem | null>(null);
  const [editName, setEditName] = useState("");
  const [editError, setEditError] = useState<string | null>(null);

  const [deleteTarget, setDeleteTarget] = useState<PersonnelCategoryItem | null>(null);
  ...
  async function handleAdd() {
    ...
    setSaving(true);
    try {
      const res = await fetch("/api/personnel-categories", {
        method: "POST",
      ...
      setCategories((prev) =>
        [...prev, { id: body.category.id, name: body.category.name, certificationCount: 0 }].sort((a, b) =>
  ...
  function openEdit(cat: PersonnelCategoryItem) {
    setEditTarget(cat);
  ...
  async function handleEdit() {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/personnel-categories/${editTarget.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/personnel-categories/${deleteTarget.id}`, { method: "DELETE" });
      if (!res.ok) {
            ...
            <div key={cat.id} className="flex items-center justify-between px-5 py-3.5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 font-mono">{cat.certificationCount} sertifikasi</p>
                </div>
              </div>
              {canManage && (
