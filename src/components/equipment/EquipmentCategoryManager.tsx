...
type EquipmentCategoryItem = { id: string; name: string; equipmentCount: number };

export function EquipmentCategoryManager({
  initialCategories,
  canManage = true,
}: {
  initialCategories: EquipmentCategoryItem[];
  canManage?: boolean;
}) {
  ...
  const [editTarget, setEditTarget] = useState<EquipmentCategoryItem | null>(null);
  const [editName, setEditName] = useState("");
  const [editError, setEditError] = useState<string | null>(null);

  const [deleteTarget, setDeleteTarget] = useState<EquipmentCategoryItem | null>(null);
  ...
  async function handleAdd() {
    ...
    try {
      ...
      setCategories((prev) =>
        [...prev, { id: body.category.id, name: body.category.name, equipmentCount: 0 }].sort((a, b) =>
  ...
  function openEdit(cat: EquipmentCategoryItem) {
    setEditTarget(cat);
  ...
  async function handleEdit() {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/equipment-categories/${editTarget.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/equipment-categories/${deleteTarget.id}`, { method: "DELETE" });
      if (!res.ok) {
            ...
            <div key={cat.id} className="flex items-center justify-between px-5 py-3.5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 font-mono">{cat.equipmentCount} alat</p>
                </div>
              </div>
              {canManage && (
