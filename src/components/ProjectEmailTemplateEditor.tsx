...
import {
  PROJECT_PLACEHOLDER_TOKENS,
  substituteProjectPlaceholders,
  buildProjectSimpleTemplateHtml,
  type ProjectSimpleTemplateFields,
} from "@/lib/project-email-template";

const SAMPLE_DATA = {
  pic: "Budi Santoso",
  projectName: "Inspeksi Kelayakan Pabrik PT Contoh",
  projectNumber: "PRJ-2026-001",
  categoryName: "Serco",
  clientName: "PT Contoh Industri",
...
export function ProjectEmailTemplateEditor({
  initialMode,
  initialSubject,
  initialSimpleFields,
  initialBodyHtml,
  ...
  initialSimpleFields: ProjectSimpleTemplateFields;
  initialBodyHtml: string;
}) {
  const { showToast } = useToast();
  const [mode, setMode] = useState<Mode>(initialMode);
  const [subject, setSubject] = useState(initialSubject);
  const [fields, setFields] = useState<ProjectSimpleTemplateFields>(initialSimpleFields);
  ...
  function updateField<K extends keyof ProjectSimpleTemplateFields>(key: K, value: string) {
  ...
  const previewHtml = useMemo(() => {
    ...
    try {
      return mode === "SIMPLE"
        ? buildProjectSimpleTemplateHtml(fields, SAMPLE_DATA, browserOrigin)
        : substituteProjectPlaceholders(bodyHtml, SAMPLE_DATA, browserOrigin);
    } catch {
  ...
  const previewSubject = useMemo(() => substituteProjectPlaceholders(subject, SAMPLE_DATA), [subject]);
  ...
  async function handleSave() {
    ...
    try {
      ...
      const res = await fetch("/api/project-email-template", {
        method: "PUT",
      ...
      showToast("Template email reminder project berhasil disimpan.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleReset() {
    setResetting(true);
    try {
      const res = await fetch("/api/project-email-template/reset", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
              ...
              <>
                <div className="rounded border border-accent/20 bg-accent/5 px-3 py-2.5 flex gap-2">
                  ...
                  <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
                    Detail project (nama, nomor, kategori, klien, target selesai) otomatis ditampilkan sebagai
                    tabel rapi di tengah email — tidak perlu diatur di sini. Anda hanya mengisi teks di
                    sekitarnya. Email ini dikirim ke PIC project + CC atasan.
                  </p>
                </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="flex flex-wrap gap-2">
              {PROJECT_PLACEHOLDER_TOKENS.map((t) => (
                <button
                  key={t.token}
